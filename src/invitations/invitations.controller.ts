...
export class InvitationsController {
  ...
  @Public()
  @ApiOperation({
    summary: "Get invitation preview (public, no auth required)",
  })
  ...
  async create(
    ...
    const response = await this.invitationsService.create(
      orgId,
      dto,
  ...
  async findForMe(@Req() req: AuthenticatedRequest) {
    const response = await this.invitationsService.findPendingForUser(
      req.user as UserEntity,
    );
    return new ApiResponse(
      true,
  ...
  @Post("accept-by-token/:token")
  @Public()
  @ApiOperation({
    summary:
      "Accept invitation by token only (no auth). For verified users before sign-in.",
  })
  async acceptByToken(@Param("token") token: string) {
    await this.invitationsService.acceptByToken(token);
    return new ApiResponse(
      true,
      HttpStatus.OK,
      "Invitation accepted successfully",
  ...
  @Post("decline-by-token/:token")
  @Public()
  @ApiOperation({
    summary:
      "Decline invitation by token only (no auth). For verified users before sign-in.",
  })
  async declineByToken(@Param("token") token: string) {
    await this.invitationsService.declineByToken(token);
    return new ApiResponse(
      true,
      HttpStatus.OK,
      "Invitation declined successfully",
