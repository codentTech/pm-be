...
export class TodoItemsService {
  ...
  private async ensureAccessToList(
    listId: string,
    userId: string,
    ...
    if (list.OrganizationId) {
      ...
      if (!isMember)
        throw new ForbiddenException(
          "You do not have access to this todo list",
        );
  ...
  private async ensureAccessToBid(
    bidId: string,
    userId: string,
  ): Promise<void> {
    ...
    if (bid.OrganizationId) {
      ...
      if (member && member.Role !== OrgRole.PROJECT_MANAGER) {
    ...
    if (dto.BidId) {
      if (!dto.DueDate) {
        throw new BadRequestException(
          "Due date is required for bid-linked todos",
        );
      }
  ...
  async findAllByTodoListIdPaginated(
    ...
    const [items, total] =
      await this.todoItemRepository.findAllByTodoListIdPaginated(
        todoListId,
        skip,
        take,
      );
  ...
  async update(
    ...
    if (Recurrence !== undefined) {
      if (Recurrence === null) {
        const existing =
          await this.todoRecurrenceRepository.findByTodoItemId(id);
  ...
  private async upsertRecurrence(
    ...
    const existing =
      await this.todoRecurrenceRepository.findByTodoItemId(todoItemId);
