...
export class BidsService {
  ...
  private async isOwnerOnlyScope(user: UserEntity, orgId: string | null): Promise<boolean> {
  ...
  private async assertCanEdit(bid: BidEntity, user: UserEntity): Promise<void> {
    if (BID_TERMINAL_STATES.includes(bid.CurrentStatus as BidStatus)) {
      throw new BadRequestException('Bid is in terminal state and cannot be edited');
    }
    if (bid.OrganizationId) {
      const isMember = await this.orgMemberRepository.isMember(user.Id, bid.OrganizationId);
  ...
  private async assertCanTransition(bid: BidEntity, user: UserEntity): Promise<void> {
    if (BID_TERMINAL_STATES.includes(bid.CurrentStatus as BidStatus)) {
  ...
  private validateTransition(from: BidStatus, to: BidStatus, dto: TransitionBidStatusDto): void {
    ...
    if (to === BidStatus.INTERVIEW && !dto.InterviewDate) {
      throw new BadRequestException('Interview date is required when moving to Interview');
    }
    if (to === BidStatus.LOST && !dto.LossReason && !dto.LossReasonOther) {
      throw new BadRequestException('Loss reason is required when marking bid as Lost');
    }
    if (to === BidStatus.LOST && dto.LossReason === BidLossReason.OTHER && !dto.LossReasonOther?.trim()) {
      throw new BadRequestException('Loss reason details required when LossReason is Other');
    }
    if (to === BidStatus.WITHDRAWN && !dto.WithdrawalReason) {
      throw new BadRequestException('Withdrawal reason is required when withdrawing bid');
    }
    if (to === BidStatus.WON && (dto.FinalAgreedPrice == null || !dto.ExpectedStartDate)) {
      throw new BadRequestException('Final agreed price and expected start date are required for Won');
    }
  }
  ...
  private appendStatusHistory(
    ...
    const history: BidStatusHistoryEntry[] = Array.isArray(bid.StatusHistory) ? [...bid.StatusHistory] : [];
    history.push({
      Status: newStatus,
  ...
  async create(dto: CreateBidDto, user: UserEntity, orgId?: string | null): Promise<BidEntity> {
    ...
    const bid = this.bidRepository.create({
      ...
      StatusHistory: [{ Status: status, Timestamp: new Date().toISOString(), UserId: user.Id }],
  ...
  async update(id: string, dto: UpdateBidDto, user: UserEntity): Promise<BidEntity> {
    const bid = await this.findOne(id, user);
    await this.assertCanEdit(bid, user);

    if (dto.CurrentStatus && dto.CurrentStatus !== bid.CurrentStatus) {
      throw new BadRequestException('Use the transition endpoint to change bid status');
    }
  ...
  async transitionStatus(id: string, dto: TransitionBidStatusDto, user: UserEntity): Promise<BidEntity> {
    const bid = await this.findOne(id, user);
    ...
    const reason = dto.LossReasonOther ?? dto.LossReason ?? dto.WithdrawalReason;
    this.appendStatusHistory(bid, to, user.Id, reason);
    bid.LastStatusAt = new Date();

    if (to === BidStatus.INTERVIEW) {
      bid.InterviewDate = dto.InterviewDate ? new Date(dto.InterviewDate) : bid.InterviewDate;
    }
    ...
    if (BID_TERMINAL_STATES.includes(to)) {
      if (from === BidStatus.INTERVIEW && !dto.InterviewOutcome && !bid.InterviewOutcome) {
        throw new BadRequestException('Interview outcome is required before terminal state');
      }
      ...
      if (to === BidStatus.WON) {
        ...
        bid.ExpectedStartDate = dto.ExpectedStartDate ? new Date(dto.ExpectedStartDate) : null;
  ...
  async bulkDelete(ids: string[], user: UserEntity, orgId?: string | null): Promise<{ deleted: number; failed: string[] }> {
  ...
  async getDraftBacklog(
    ...
    const cutoffDate = new Date(Date.now() - thresholds.draftAgingDays * 24 * 60 * 60 * 1000);
    const bids = await this.bidRepository.findAllByOrg(
      resolvedOrgId,
      ownerOnly ? user.Id : undefined,
    );
    const filtered = bids
      .filter((b) => b.CurrentStatus === BidStatus.DRAFT && b.CreatedAt <= cutoffDate)
  ...
  async getFollowUpBacklog(
    ...
    const ownerOnly = await this.isOwnerOnlyScope(user, resolvedOrgId);
    const thresholds = await this.getThresholds(resolvedOrgId);
    const cutoffDate = new Date(Date.now() - thresholds.followUpSlaDays * 24 * 60 * 60 * 1000);
    const bids = await this.bidRepository.findAllByOrg(
      resolvedOrgId,
      ownerOnly ? user.Id : undefined,
    );
    const filtered = bids
      .filter((b) =>
        [BidStatus.SUBMITTED, BidStatus.INTERVIEW].includes(b.CurrentStatus as BidStatus),
      )
      .filter((b) => (b.LastStatusAt ?? b.CreatedAt) <= cutoffDate)
      .sort((a, b) => (a.LastStatusAt ?? a.CreatedAt).getTime() - (b.LastStatusAt ?? b.CreatedAt).getTime());
    const take = Math.min(100, Math.max(1, limit));
    const skip = (Math.max(1, page) - 1) * take;
    const items = filtered.slice(skip, skip + take);
    return createPaginatedResponse(items, filtered.length, page, take);
  ...
  async getGhostedSuggestions(
    ...
    const ownerOnly = await this.isOwnerOnlyScope(user, resolvedOrgId);
    const thresholds = await this.getThresholds(resolvedOrgId);
    const cutoffDate = new Date(Date.now() - thresholds.ghostedSuggestDays * 24 * 60 * 60 * 1000);
    const bids = await this.bidRepository.findAllByOrg(
      resolvedOrgId,
      ownerOnly ? user.Id : undefined,
    );
  ...
  async getMetrics(user: UserEntity, orgId: string | null | undefined) {
    try {
      ...
      const avgDraftAgeDays =
        draftBids.length > 0
          ? draftBids.reduce((sum, b) => sum + (now - b.CreatedAt.getTime()), 0) /
            draftBids.length /
            86400000
          : null;
      ...
      const avgDealSize =
        wonBidsWithPrice.length > 0
          ? wonBidsWithPrice.reduce((sum, b) => sum + Number(b.FinalAgreedPrice), 0) /
      ...
      const followUpOverdueCount = bids.filter((b) => {
        ...
        if (![BidStatus.SUBMITTED, BidStatus.INTERVIEW].includes(status)) return false;
      ...
      const ghostedSuggestedCount = bids.filter((b) => {
        const status = b.CurrentStatus as BidStatus;
        if (![BidStatus.SUBMITTED, BidStatus.VIEWED, BidStatus.INTERVIEW].includes(status))
          return false;
        const last = (b.LastStatusAt ?? b.CreatedAt).getTime();
